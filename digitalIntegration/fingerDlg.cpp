...
void fingerDlg::DoRegister(unsigned char* temp, int len)
{
	...
	if (m_enrollIdx > 0)
	{
		...
		if (0 >= ZKFPM_DBMatch(hDBCache, arrPreRegTemps[m_enrollIdx - 1], arrPreTempsLen[m_enrollIdx - 1], temp, len))
		{
			m_enrollIdx = 0;
			m_bRegister = FALSE;
			MessageBox(NULL, TEXT("注册时，请按相同的手指"), TEXT("提示"), 0);
		...
		if (ZKFP_ERR_OK == ret)   //ZKFP_ERR_OK=操作成功
		{
			...
			if (ZKFP_ERR_OK == ret)
			{
				...
				//nLastRegTempLen = cbRegTemp; 
					...
					if (db::databaseDI::Instance().add_user_finger(szRegTemp, cbRegTemp))
					{
...
{	
	//if (nLastRegTempLen > 0)	//have enroll one more template
	{
