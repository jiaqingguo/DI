...
public: 
	...
	{   
		// 获取宽字符的长度
		size_t length = std::wcslen(szWidechar);

		// 计算所需的字节数
		size_t bufferSize = (length + 1) * sizeof(char);

		// 创建用于存储转换后字符串的缓冲区
		char* szChar = new char[bufferSize];

		// 宽字符到普通字符转换
		std::wcstombs(szChar, szWidechar, bufferSize);
