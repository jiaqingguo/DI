...
int Server::sendFileData(SOCKET datatcps, std::ifstream& file)
{
  ...
  // // 使用 memcpy 将 int 的字节复制到 char 数组
		...
		sizeBuffer[0] = static_cast<unsigned char>(data_size & 0xFF); // 低位
		sizeBuffer[1] = static_cast<unsigned char>((data_size >> 8) & 0xFF); // 高位

		
// 使用 memcpy 将 int 的字节复制到 char 数组
		
		int sendSize = send(datatcps, (char*)sizeBuffer, sizeof(sizeBuffer), 0);
		cout << "send int value  " << sizeBuffer << " " << sendSize << endl;
		sendSize = send(datatcps, buffer, data_size, 0);
		cout << "send buffer value  " << strlen(buffer) << " " << sendSize << endl;
	}
	...
	unsigned char sizeBuffer[2];
	sizeBuffer[0] = static_cast<unsigned char>(end_signal & 0xFF); // 低位
	sizeBuffer[1] = static_cast<unsigned char>((end_signal >> 8) & 0xFF); // 高位
	send(datatcps, (char*)sizeBuffer, sizeof(sizeBuffer), 0);
	cout << "发送结束0" << sizeBuffer << endl;
	return 1;
}
